(function() {
  let s = document.getElementById('arith');
  if (!s) {
  ...
  const d = document.querySelector('.arith'),
  ...
  function addQuiz() {
    ...
    if (t === '-' && v2 > v1) {
      const v = v2; v2 = v1; v1 = v;
    }
    // 保证能整除，不保证第一个数字能符合位数设定
    if (t === '÷') {
      const v = Math.ceil(v1 / v2) * v2;
  ...
  const lb = ['继续', '再来', '加油', '好的', '走你', '嘿哟', '挺住', '瞄瞄', '汪汪', '服了', '可以', '满分', '聪明', '优秀', '不错', '行啊', '厉害', '真棒', 'Good', 'Perfect', 'Excellent', '你牛', '佩服', '哎呀'];
  // 输入答案后验证是否正确
  res.addEventListener('change', function(e) {
    const t = type.value, ans = t === '+' ? v1 + v2 : (
      t === '-' ? v1 - v2 : (
        t === '×' ? v1 * v2 : (
          t === '÷' && v1 / v2)));
